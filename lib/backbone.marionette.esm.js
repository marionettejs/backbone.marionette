...
var UIMixin = {
  ...
  normalizeUIKeys: function normalizeUIKeys(hash) {
    ...
    return _.reduce(hash, function (memo, val, key) {
      var normalizedKey = _normalizeUIString(key, uiBindings);
...
function normalizeRegion (definition, regionClass) {
...
var ClassOptions$1 = ['currentView', 'el', 'replaceElement'];
...
_.extend(Region.prototype, CommonMixin, {
  ...
  setElement: function setElement(el) {
    ...
    var replaceElement = this._isReplaced;
    ...
    if (this.currentView) {
      ...
      this._attachView(this.currentView, {
  ...
  _ensureElement: function _ensureElement() {
    if (!this.el) {
      throw new MarionetteError({
        name: classErrorName,
        message: "A region el is required. Region cid:".concat(this.cid),
...
function startsWithUi(el) {
  return el[0] === '@' && el[1] === 'u' && el[2] === 'i' && el[3] === '.';
} // MixinOptions
// - regions
// - regionClass
...
var RegionsMixin = {
  ...
  _initRegions: function _initRegions() {
    ...
    this._regions = {}; // init regions definition hash

    this.regions = _.extend({}, _.result(this, 'regions'));
  },
  // Adds or rebinds all regions associated with the view
  _bindRegions: function _bindRegions() {
    var regions = this.addRegions(_.extend({}, this.regions));
    this.triggerMethod('bind:regions', this, regions);
  },
  _buildRegion: function _buildRegion(name, RegionClass, options) {
  ...
  _addRegion: function _addRegion(options, name) {
    var currentRegion = this.getRegion(name);
  ...
  _buildOptions: function _buildOptions(definition, name) {
    var options = normalizeRegion(definition, this.regionClass);
  ...
  addRegion: function addRegion(name, definition) {
    ...
    var options = this._buildOptions(definition, name);

    return this._addRegion(options, name);
  },
  // Add multiple region definitions
  addRegions: function addRegions(regionDefinitions) {
    var _this = this;
    ...
    var regionOptions = _.reduce(regionDefinitions, function (opts, definition, name) {
      opts[name] = _this._buildOptions(definition, name);
      return opts;
    }, {});
...
var View = Backbone.View.extend({
  ...
  setElement: function setElement() {
    var canReady = !this._isRendered && this._isInited;
  ...
  render: function render() {
    ...
    var wasRendered = this._isRendered;
