...
Marionette.View = Backbone.View.extend({
  ...
  configureTriggers: function(existingEvents){
    ...
    _.each(triggers, function(value, key){
      ...
      // if we do not already have an existing function that should be called
      // on this event, due to an entry in the events hash, then just add the trigger
      // function to the triggerEvents hash (which will later be merged with the
      // original events hash). But if there is already an exiting function there, then
      // we need to compose a new function that calls the existing function as well
      // as our new function. Otherwise the existing fucntion would never get called.
      if( ! existingEvents || ! existingEvents[key] )
        triggerEvents[key] = triggerFunction;
      else
      {
        var existingMethod = existingEvents[key];
        if (!_.isFunction(existingMethod)) existingMethod = that[existingEvents[key]];
        if (!existingMethod) throw new Error('Method "' + existingEvents[key] + '" does not exist');

        triggerEvents[key] = _.compose( triggerFunction, existingMethod );
      }
    });
  ...
  delegateEvents: function(events){
    ...
    var combinedEvents = {};
    var triggers = this.configureTriggers(events);
